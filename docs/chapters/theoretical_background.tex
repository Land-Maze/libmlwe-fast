\chapter{Theoretical Background of Module-LWE and Number Theoretic Transform}
\label{ch:theoretical_background}

This chapter establishes the mathematical and computational foundations necessary for this thesis. It begins by defining the complexity classes that underpin modern cryptography, analyzes the specific threats posed by quantum computing, and finally details the lattice-based structures and algorithms, specifically M-LWE and the NTT, that enable secure and efficient post-quantum communication.
...
To understand "infeasibility," the underlying computational complexity classes must be defined.
...
\begin{figure}[htbp]
    ...
    \caption{Bloch Sphere Visualisation. \newline [Source: Nielsen \& Chuang~\cite{nielsen2010quantum}]}
...
Two primary quantum algorithms necessitate the transition to post-quantum cryptography:

\subsubsection{Shor's Algorithm}
Peter Shor introduced an algorithm~\cite{shor1994algorithms} that solves Integer Factorization and Discrete Logarithm problems in polynomial time.
...
This effectively breaks RSA and ECC.
...
In response to the quantum threat, NIST has standardized Lattice-based cryptography.
...
\begin{figure}[htbp]
    ...
    \caption{Visualisation of a 2D Lattice. \newline [Source: "What is Lattice-based Cryptography?". Utimaco. https://utimaco.com/service/knowledge-base/post-quantum-cryptography/what-lattice-based-cryptography]}
...
The LWE problem, introduced by Regev~\cite{regev2005lattices}, is essentially the cryptographic realization of the CVP.
...
To achieve the "Fast Implementation" goal of this thesis, we utilize M-LWE~\cite{langlois2015worst}. M-LWE balances the high security of standard LWE with the efficiency of structured variants like Ring-LWE~\cite{LyubashevskyPR13}.
...
Computations are defined over the ring $R_q = \mathbb{Z}_q[X] / \langle X^d + 1 \rangle$, where $d=256$ is a power of 2.
In this setting, elements are not single numbers but polynomials of degree $d-1$.
This structure allows the public matrix $A$ to be represented in a compressed form, significantly reducing memory usage from kilobyte-sized keys to compact polynomial representations.
...
Additionally, the matrix-vector multiplication in M-LWE involves polynomial multiplications, which can be optimized using the Number Theoretic Transform, as discussed in Section~\ref{sec:ntt}.
...
The parameter $k$ allows security to be tuned: Kyber-512 uses $k=2$, while Kyber-768 uses $k=3$.
With increased $k$, the security level rises, but so does computational complexity.

\section{The Number Theoretic Transform (NTT)}
\label{sec:ntt}

The NTT is the critical algorithmic driver for high-performance M-LWE implementations.
...
Instead, the scheme utilizes the \textbf{negacyclic NTT}, also known as the negative wrapped convolution.
...
By utilizing this specialized transform, the polynomial multiplication bottleneck can be performed in $O(d \log d)$ time without doubling the length of the input vectors through zero-padding.
...
\section{Summary for Chapter 1}
\label{sec:summary_chapter_1}

In summary, this chapter established the theoretical framework for the thesis. The discussion reviewed the computational hardness assumptions that underpin modern cryptography and analyzed the centrality of the P vs NP problem to security. The existential threat posed by quantum algorithms, specifically those of Shor and Grover, was identified, presenting lattice-based cryptography as a robust solution. The transition from standard LWE to Module-LWE was detailed, highlighting how the algebraic structure of polynomial rings offers superior computational efficiency. Finally, the NTT was introduced as the essential mathematical tool for optimizing these schemes. The subsequent chapters build upon these foundations to design, implement, and evaluate the high-performance Module-LWE library.
